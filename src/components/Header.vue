    ...
    <div class="workingSpace">
      <div class="loggedinDiv">
        <div v-if="state.loggedin" class="loggedInTitle">
          ...
          <a :href="state.baseURL + '/?i=/u/' + state.loggedinUserName" v-html="truncate(state.loggedinUserName)"></a>
        </div>
        <Account :state="state"/>
      </div>
...
export default {
  ...
  methods:{
    launchInfoPage(){
      ...
      a.setAttribute('href', 'https://code.whitehot.ninja/a/about/')
    ...
    createDemo(){
      ...
      fetch(this.state.baseURL + '/createDemo.php',{
        ...
        if(data[0]) window.location.href = this.state.baseURL + '/?i=/d/' + this.state.decToAlpha(data[1])
      })
    }
  },
  mounted(){
  ...
  computed:{
    ...
    origin(){
      return window.location.origin+"/a/?i="
    }
  }
}
</script>
...
.appName{
  ...
  background: #1350 url(https://srmcgann.github.io/assets/1GY3GM.png) no-repeat;
...
.infoButton{
  ...
  margin-right: 5px;
  float: right;
  background-image: url(https://srmcgann.github.io/assets/WEOZD.png);
  background-repeat: no-repeat;
...
.jumpButton{
  background-image:url(https://srmcgann.github.io/assets/1RptlQ.png);
